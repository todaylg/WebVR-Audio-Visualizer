...
AFRAME.registerComponent('audioanalyser', {
	schema: {
		...
		fftSize: { default: 2048 },//fftSize 属性的值必须是从32到32768范围内的2的非零幂; 其默认值为2048.
	...
	update: function () {
		...
		function init(analyser) {
			...
			self.levels = new Uint8Array(self.analyser.frequencyBinCount);//fftSize值的一半. 该属性通常用于可视化的数据值的数量.(柱状频谱)
			self.waveform = new Uint8Array(self.analyser.fftSize);//FFT (快速傅里叶变换) 的大小.(波形频谱)
